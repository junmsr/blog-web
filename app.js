/* ══════════════════════════════════════════════════════
   VOID.BLOG — app.js
   All posts stored in Supabase PostgreSQL.
   Share links work on any device, any browser.
   ══════════════════════════════════════════════════════ */

'use strict';

// ── 1. Guard: Supabase SDK must be loaded ──────────────
if (typeof supabase === 'undefined') {
  document.body.innerHTML = `
    <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;
      background:#080b10;font-family:monospace;text-align:center;padding:2rem;">
      <div>
        <div style="font-size:2rem;margin-bottom:1rem;">⚠️</div>
        <div style="color:#ff2d78;font-size:1rem;margin-bottom:0.75rem;">
          Supabase SDK failed to load.</div>
        <div style="color:#5a6478;font-size:0.8rem;line-height:1.8;">
          You must open this site through a web server, not a local file.<br>
          In VS Code: right-click index.html → <strong style="color:#00f5d4;">Open with Live Server</strong><br>
          Or deploy to Netlify / Vercel / GitHub Pages.
        </div>
      </div>
    </div>`;
  throw new Error('Supabase SDK not loaded — open via a web server, not file://');
}

// ── 2. Guard: credentials must be filled in ────────────
if (!window.SUPABASE_URL  || window.SUPABASE_URL.includes('YOUR_') ||
    !window.SUPABASE_ANON || window.SUPABASE_ANON.includes('YOUR_')) {
  document.body.innerHTML = `
    <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;
      background:#080b10;font-family:monospace;text-align:center;padding:2rem;">
      <div>
        <div style="font-size:2rem;margin-bottom:1rem;">🛠</div>
        <div style="color:#f5c518;font-size:1rem;margin-bottom:0.75rem;">
          Supabase credentials not configured.</div>
        <div style="color:#8892a4;font-size:0.78rem;line-height:2;">
          Open <strong style="color:#00f5d4;">index.html</strong> and replace:<br>
          <code style="color:#00f5d4;">YOUR_SUPABASE_PROJECT_URL</code><br>
          <code style="color:#00f5d4;">YOUR_SUPABASE_ANON_KEY</code><br>
          with your real values from<br>
          <strong style="color:#00f5d4;">supabase.com → Project Settings → API</strong>
        </div>
      </div>
    </div>`;
  throw new Error('Supabase credentials are placeholder values — update index.html');
}
...
// ── 4. App state ───────────────────────────────────────
const KEY_AUTH = 'vb_authed';   // sessionStorage only — no posts in localStorage
...
async function boot() {
  ...
  // Restore owner session first so hash detection works correctly
  ...
  if (postId && !isOwner) {
    // External viewer arriving via share link
    isViewerMode = true;
    applyMode();
    await loadAbout();
    updateAboutSidebar();
  ...
  if (postId && isOwner) {
    // Owner reloaded on a post page — open it in owner mode, clean the hash
  ...
  const { data: pwdRow } = await db
    .from('settings').select('value').eq('key', 'owner_pwd_hash').maybeSingle();

  if (!pwdRow)      showSetupModal();
  else if (!isOwner) openPwdModal();
...
function showLoading(show) {
  ...
  if (!el) {
    ...
    el.innerHTML = '<div class="vb-spinner"></div>';
    Object.assign(el.style, {
      position:'fixed', inset:'0', background:'rgba(8,11,16,0.92)',
      display:'flex', alignItems:'center', justifyContent:'center',
      zIndex:'9000', transition:'opacity 0.3s',
    });
    const s = document.createElement('style');
    s.textContent = `
      .vb-spinner{width:36px;height:36px;border:2px solid rgba(0,245,212,.15);
        border-top-color:#00f5d4;border-radius:50%;animation:vbspin .7s linear infinite}
...
async function loadAbout() {
  ...
  about = {};
  (data || []).forEach(r => { about[r.key.replace('about_', '')] = r.value; });
  if (!about.name)   about.name   = 'Author';
  if (!about.handle) about.handle = '@void';
  if (!about.bio)    about.bio    = 'Welcome to my blog.';
}
...
/* ════════════════════════════════════════════════════════
   DATABASE — WRITE
   ════════════════════════════════════════════════════════ */
async function dbSavePost(payload, id = null) {
  const row = {
    title:      payload.title,
...
async function uploadImage(base64, path) {
  ...
  const fname = `${path}-${Date.now()}.${ext}`;
  const { error } = await db.storage
    .from('blog-assets').upload(fname, blob, { upsert: true, contentType: mime });
  if (error) throw error;
  const { data } = db.storage.from('blog-assets').getPublicUrl(fname);
...
function showSetupModal() {
  ...
  document.getElementById('pwd-modal-desc').textContent   =
    'Create a password to protect owner access. Anyone with a share link gets view-only access — no login needed.';
...
async function tryLogin() {
  ...
  if (isSetupMode) {
    try {
      ...
      closePwdModal(); applyMode();
      await loadPosts(); renderHome();
      toast("Password set! You're the owner ⚡", 'success');
    } catch (e) {
      errEl.textContent   = 'Could not save — check your Supabase connection.';
      errEl.style.display = 'block';
      console.error('[tryLogin setup]', e);
    }
    return;
  }
  ...
  try {
    ...
    errEl.textContent   = 'Connection error — check your Supabase config.';
    errEl.style.display = 'block';
...
function logout() {
  ...
  isOwner = false;
  applyMode(); renderHome(); showView('home');
...
function applyMode() {
  ...
  if (isViewerMode) {
    ...
    set('owner-badge','none'); set('nav-write','none');
    set('logout-btn','none');  set('login-btn-small','none');
    set('nav-about','none');
  } else if (isOwner) {
    banner && banner.classList.remove('visible');
    set('owner-badge','inline-flex'); set('nav-write','inline-flex');
    set('logout-btn','inline-flex');  set('login-btn-small','none');
    set('nav-about','');
  } else {
    banner && banner.classList.remove('visible');
    set('owner-badge','none'); set('nav-write','none');
    set('logout-btn','none');  set('login-btn-small','inline-flex');
    set('nav-about','');
  }
}
...
function showView(name) {
  ...
  // Clear viewer mode when owner navigates away
  if (isOwner && isViewerMode) { isViewerMode = false; applyMode(); }
...
function renderHome() {
  ...
  const drafts = isOwner
    ...
    : [];
  const totalWords = posts.reduce((a, p) => a + wc(p.content), 0);

  document.getElementById('stats-row').innerHTML = `
    <div class="stat-box"><div class="stat-num">${published.length}</div><div class="stat-label">Posts</div></div>
    ${isOwner ? `<div class="stat-box"><div class="stat-num">${drafts.length}</div><div class="stat-label">Drafts</div></div>` : ''}
    <div class="stat-box"><div class="stat-num">${(totalWords/1000).toFixed(1)}k</div><div class="stat-label">Words</div></div>`;
  ...
  if (!published.length && !drafts.length) {
    c.innerHTML = `
      <div class="empty-state">
        ...
        ${isOwner
          ? `<button class="btn-primary" onclick="showView('write')"
               style="width:auto;display:inline-flex;gap:.4rem;padding:.75rem 1.5rem;">⚡ Write Now</button>`
          : ''}
      </div>`;
    return;
  }
    ...
    html += `
      <div class="featured-card" onclick="openPost('${f.id}')">
        <div class="featured-content">
          ...
          <div class="card-tags">${chips(f.tags)}</div>
          <div class="featured-title">${esc(f.title)}</div>
          <div class="featured-excerpt">${esc(f.excerpt || stripMd(f.content).slice(0,200))}</div>
          <div class="featured-meta">${fmtDate(f.createdAt)}<span>·</span>${readTime(f.content)} min read</div>
        </div>
        <div class="featured-cover">
          ${f.cover ? `<img src="${f.cover}" alt="Cover">` : `<span class="cover-placeholder">${(f.title[0]||'◌').toUpperCase()}</span>`}
        </div>
      </div>`;
        ...
        html += `
          <div class="post-card" onclick="openPost('${p.id}')">
            <div class="post-card-cover">
              ${p.cover ? `<img src="${p.cover}" alt="">` : `<span class="post-card-placeholder">${(p.title[0]||'◌').toUpperCase()}</span>`}
            </div>
            <div class="post-card-body">
              <div class="card-tags" style="margin-bottom:.6rem">${chips(p.tags,2)}</div>
      ...
      html += `
        <div class="draft-card" onclick="editPost('${d.id}')">
          ...
          <div>
            ...
            <div class="draft-meta">${fmtDate(d.createdAt)} · ${wc(d.content)} words</div>
          </div>
...
/* ════════════════════════════════════════════════════════
   OPEN POST — always fetches live from Supabase
   This is what makes share links work on any device.
   ════════════════════════════════════════════════════════ */
async function openPost(id) {
  ...
  if (error) {
    console.error('[openPost] DB error:', error);
    return showPostError(`DB error: ${error.message} (${error.code})`);
  }
  if (!row) {
    console.warn('[openPost] No post found for id:', id);
    return showPostError(null);
  }
  ...
  // Only write the hash when the owner views a post — creates the shareable URL
  if (isOwner) window.location.hash = `#post/${id}`;

  const viewerBanner  = isViewerMode
  ...
  const ownerControls = isOwner ? `
    <div class="pv-action-group">
      <button class="btn-outline" onclick="editPost('${post.id}')"
        style="width:auto;margin:0;padding:.5rem .9rem;font-size:.62rem;">✏️ Edit</button>
  ...
  document.getElementById('pv-content').innerHTML = `
    ...
    ${viewerBanner}
    <div class="pv-tags">${chips(post.tags)}</div>
    <h1 class="pv-title">${esc(post.title)}</h1>
    ${post.excerpt ? `<p class="pv-subtitle">${esc(post.excerpt)}</p>` : ''}
    <div class="pv-meta">
      <span>${fmtDate(post.createdAt)}</span><span>·</span>
      <span>${readTime(post.content)} min read</span><span>·</span>
      <span>${wc(post.content)} words</span>
    </div>
    ${post.cover ? `<img class="pv-hero" src="${post.cover}" alt="Cover">` : ''}
    <div class="pv-body">${renderMarkdown(post.content)}</div>
    <div class="pv-actions">
      ${!isViewerMode ? `<button class="pv-back" onclick="showView('home')">← All posts</button>` : '<div></div>'}
      ${ownerControls}
    </div>
    ${shareBlock}`;
...
function showPostError(debugMsg) {
  const detail = debugMsg
    ? `<div style="font-family:'Space Mono',monospace;font-size:.65rem;color:var(--pink);
         margin-top:1rem;padding:.75rem 1rem;background:var(--pink-dim);
         border:1px solid rgba(255,45,120,.2);border-radius:8px;text-align:left;">
         ${esc(debugMsg)}</div>`
    : `<p style="font-size:.85rem;color:var(--muted);margin-top:.5rem;">
         This post may have been removed or the link has expired.</p>`;
  document.getElementById('pv-content').innerHTML = `
    <div style="text-align:center;padding:6rem 2rem;">
      <div style="font-size:3rem;opacity:.2;margin-bottom:1rem;">◌</div>
      <div style="font-family:'Syne',sans-serif;font-size:1.5rem;font-weight:800;color:var(--muted2);">
        Post not found.</div>
      ${detail}
      ${!isViewerMode
        ? `<button class="pv-back" onclick="showView('home')" style="margin-top:2rem;">← Back to feed</button>`
        : ''}
    </div>`;
  document.querySelectorAll('.view').forEach(v => v.classList.remove('active'));
  document.getElementById('view-post').classList.add('active');
...
function copyLink(url) {
  navigator.clipboard.writeText(url)
    ...
    .catch(() => toast('Copy failed — paste it manually.', 'error'));
}
...
async function savePost(status) {
  ...
  if (coverData && coverData.startsWith('data:')) {
    try {
      const path = editingId ? `covers/${editingId}` : `covers/new-${Date.now()}`;
      coverUrl   = await uploadImage(coverData, path);
    } catch (e) {
      console.error('[savePost] image upload:', e);
  ...
  try {
    showLoading(true);
    const saved = await dbSavePost(
      { title, excerpt, content, tags: [...tags], cover: coverUrl, status },
      editingId || null
    );
    if (editingId) {
...
coverZone.addEventListener('drop', e => {
  e.preventDefault(); coverZone.classList.remove('over');
  const f = e.dataTransfer.files[0];
  if (f && f.type.startsWith('image/')) readCoverFile(f);
});
...
document.getElementById('tag-input').addEventListener('keydown', function(e) {
  if ((e.key === 'Enter' || e.key === ',') && this.value.trim()) {
    e.preventDefault(); addTag(this.value.trim().replace(/,/g,'')); this.value = '';
  }
  if (e.key === 'Backspace' && !this.value && tags.length) removeTag(tags[tags.length-1]);
});
function addTag(v)    { if (!v || tags.includes(v)) return; tags.push(v); renderTagsUI(); }
function removeTag(v) { tags = tags.filter(t => t !== v); renderTagsUI(); }
...
function chips(arr, limit = null) {
  if (!arr || !arr.length) return '<span class="card-tag">Essay</span>';
  return (limit ? arr.slice(0, limit) : arr).map(t => `<span class="card-tag">${esc(t)}</span>`).join('');
}
...
function updateWordCount() {
  const n = wc(document.getElementById('post-content').value);
...
/* ════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════ */
function ins(b, a) {
  const ta = document.getElementById('post-content'), s = ta.selectionStart, e = ta.selectionEnd;
  ta.setRangeText(b + ta.value.slice(s,e) + a, s, e, 'select'); ta.focus();
}
function insLine(p) {
  const ta = document.getElementById('post-content');
  const ls = ta.value.lastIndexOf('\n', ta.selectionStart-1)+1;
  ta.setRangeText(p, ls, ls, 'end'); ta.focus();
}
function insBlock(b, a) {
  const ta = document.getElementById('post-content'), s = ta.selectionStart, e = ta.selectionEnd;
  ta.setRangeText(b + ta.value.slice(s,e) + a, s, e, 'end'); ta.focus();
}
...
function renderAbout() {
  ...
  if (av) av.innerHTML = about.avatar
    ? `<img src="${about.avatar}" alt="Avatar">`
    : (about.name||'A')[0].toUpperCase();
  const wrap = document.getElementById('about-edit-wrap');
  if (!wrap) return;
...
function updateAboutSidebar() {
  ...
  if (desc && about.bio)
    desc.textContent = about.bio.length > 130 ? about.bio.slice(0,130)+'...' : about.bio;
}
...
async function saveAbout() {
  ...
  try {
    ...
    showLoading(false); updateAboutSidebar(); renderAbout();
...
function stripMd(s)  { return (s||'').replace(/[#*`_~>]/g,'').replace(/\n/g,' ').trim(); }
function wc(t)       { return (t||'').trim().split(/\s+/).filter(Boolean).length; }
function readTime(t) { return Math.max(1, Math.round(wc(t)/200)); }
function fmtDate(ts) {
